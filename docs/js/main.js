...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "default": () => (/* binding */ ExifUtil)
/* harmony export */ });
...
var ExifUtil = /*#__PURE__*/function () {
  ...
  return _createClass(ExifUtil, [{
    key: "getFieldNameLabel",
    value: function getFieldNameLabel(key) {
      if (key == null) {
        return key;
      }
      var hasProp = isObject(this.FieldName) && Object.prototype.hasOwnProperty.call(this.FieldName, key);
    ...
    value: function getExifValueLabel(key, value) {
      ...
      var hasFormat = isObject(this.ValueFormat) && Object.prototype.hasOwnProperty.call(this.ValueFormat, key);
      if (!hasFormat) {
        return value;
      }
      var format = this.ValueFormat[key];
      if (!isObject(format)) {
        return value;
      }
      var formatType = format.type;
      var formatLabel = format.label;

      // Fast-path early returns to avoid unnecessary variable re-assignments
      // and switch evaluation overhead
      if (formatType === 'replace') {
        var hasReplaced = isObject(formatLabel) && Object.prototype.hasOwnProperty.call(formatLabel, value);
    ...
    value: function isSupport(mimeType) {
      ...
      var normalizedType = mimeType.trim().toLowerCase().split(';')[0];
...
/* harmony import */ var _exif_util__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./exif_util */ "./exif_util.js");
...
jquery__WEBPACK_IMPORTED_MODULE_0___default()(function () {
  ...
  var exifUtil = new _exif_util__WEBPACK_IMPORTED_MODULE_3__["default"](_config__WEBPACK_IMPORTED_MODULE_2__["default"].FieldName, _config__WEBPACK_IMPORTED_MODULE_2__["default"].valueFormat);
  if (!window.FileReader) {
    window.alert('File API がサポートされていません。');
    return false;
  }
  var $overlay = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.overlay');
  var $dropArea = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.dropArea');
  var $exifInfo = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.exifInfo');
  var $previewArea = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.previewArea');
  ...
  var analyzeExif = function analyzeExif(file, $infoBox, $imageBox) {
    ...
    if (!_exif_util__WEBPACK_IMPORTED_MODULE_3__["default"].isSupport(file.type)) {
      $infoBox.text('サポートされていない形式です。');
      return;
    }
    try {
      // EXIF.getData 自体の同期的な呼び出しエラーをキャッチ
      exif_js__WEBPACK_IMPORTED_MODULE_1___default().getData(file, function () {
        try {
          // 非同期コールバック内で EXIF タグ解析時の例外をキャッチ
          var exif = exif_js__WEBPACK_IMPORTED_MODULE_1___default().getAllTags(file);
          if (!exif) {
    ...
    try {
      var img = new Image();
      var _cleanup = function cleanup(event) {
        if (windowURL && typeof windowURL.revokeObjectURL === 'function') {
          windowURL.revokeObjectURL(event.target.src);
        }
        event.target.removeEventListener('load', _cleanup);
        event.target.removeEventListener('error', _cleanup);
      };
      var safeName = _exif_util__WEBPACK_IMPORTED_MODULE_3__["default"].sanitizeFileName(file ? file.name : '');
      img.alt = safeName;
      img.title = safeName;
      img.src = windowURL.createObjectURL(file);
      img.addEventListener('load', _cleanup);
      img.addEventListener('error', _cleanup);
      $imageBox.append(img);
    } catch (error) {
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('body').on('dragenter', function (event) {
    $dropArea.addClass('dropping');
    $overlay.show();
    return cancelEvent(event);
  }).on('dragover', cancelEvent).on('drop', function (event) {
    $exifInfo.empty();
    $previewArea.empty();
